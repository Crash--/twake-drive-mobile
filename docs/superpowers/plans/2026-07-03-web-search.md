...
```ts
import { searchFilesQuery, searchFilesQueryAs } from './queries'

describe('searchFilesQuery', () => {
  it('cible io.cozy.files avec un $regex insensible à la casse sur name, hors corbeille', () => {
    const q = searchFilesQuery('report') as unknown as {
      doctype: string
      selector: { name: { $regex: RegExp }; trashed: unknown }
      indexedFields: string[]
      sort: unknown
      limit: number
    }
    expect(q.doctype).toBe('io.cozy.files')
    expect(q.selector.name.$regex).toBeInstanceOf(RegExp)
    expect(q.selector.name.$regex.flags).toContain('i')
    expect(q.selector.name.$regex.test('Q3 REPORT.pdf')).toBe(true)
    expect(q.selector.trashed).toEqual({ $ne: true })
    expect(q.indexedFields).toEqual(['name'])
    expect(q.sort).toEqual([{ name: 'asc' }])
    expect(q.limit).toBe(50)
  })

  it('échappe les métacaractères de la saisie', () => {
    const q = searchFilesQuery('a.b') as unknown as { selector: { name: { $regex: RegExp } } }
    expect(q.selector.name.$regex.test('axb')).toBe(false)
    expect(q.selector.name.$regex.test('a.b')).toBe(true)
  })
